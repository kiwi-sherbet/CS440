...
class pixel_group:
    def __init__(self):
        self.number = R**(N*M)
        self.case = [0 for i in range(0, self.number)]
...
def compute_tot_foreground(training_labels_file, training_images_file):
    ...
    total_ra = [[[pixel_group() for cls in range(0,NUM_CLASSES)] for j in range(0,WIDTH)] for i in range(0,HEIGHT)]
        ...
        for i in range(0,HEIGHT):
            for j in range(0,WIDTH):
                ...
                for m in range(0,M):
                    for n in range(0,N):
                        if OVERLAPPING:
                            t = is_foreground(curr_img[i+m][j+n])
                            case_class += t*(R**(n*M+m))
                        else:
                            t = is_foreground(curr_img[M*i+m][N*j+n])
                            case_class += t*(R**(n*M+m))
                total_ra[i][j][cls].case[case_class] += 1
    return total_ra

def compute_likelihood(total_ra, ex_per_class_ra, k):
    ...
    likelihood_ra = [[[pixel_group() for cls in range(0,NUM_CLASSES)] for j in range(0,WIDTH)] for i in range(0,HEIGHT)]
    for cls in range(0,NUM_CLASSES):
        for n in range(0, NUM_CLASSES):
            ...
            for i in range(0, HEIGHT):
                for j in range(0,WIDTH):
                    ###########################################################
                    for case_index in range(0, likelihood_ra[i][j][cls].number):
                        likelihood_ra[i][j][cls].case[case_index] = (float(total_ra[i][j][cls].case[case_index]) + k) / (float(num_ex_per_class) + k*V)
                    ###########################################################
    return likelihood_ra
...
def training(training_labels_file, training_images_file, k):
    ...
    total_ra = compute_tot_foreground(training_labels_file, training_images_file)
    likelihood_ra = compute_likelihood(total_ra, ex_per_class_ra, k)
    return priors_ra, likelihood_ra

def testing(priors_ra, likelihood_ra, test_images_file, test_labels_file):
    """Performs maximum a priori (MAP) classification of test images
        according to the learned Naive Bayes model.

    Arguments:
        priors_ra: length-10 array, where value at index if is prior for digit i
                    ...
                    for m in range(0,M):
                        for n in range(0,N):
                            if OVERLAPPING:
                                ...
                                t = is_foreground(curr_img[M*i+m][N*j+n])
                                case_class += t*(R**(n*M+m))
                    posterior += math.log(likelihood_ra[i][j][cls].case[case_class])
...
def compute_rate_correct(estimated_labels, actual_labels):
...
def print_confusion_matrix(matrix):
    """Prints a 10^(NxM)x10^(NxM) matrix of confusion rates between classes.

    Arguments:
        matrix: the 10^(NxM)x10^(NxM) matrix of confusion rates
    """
    for i in range(0,NUM_CLASSES^(N*M)):
        for j in range(0,NUM_CLASSES^(N*M)):
            print('{0:0.10f}'.format(matrix[i][j])),
        print('')

def compute_confusion_matrix(estimated_labels, actual_labels): # change NUM_CLASSES => NUM_CLASSES^(NxM)
    ...
    Returns:
        confusion_matrix: 10^(NxM)x10^(NxM) matrix of confusion rates between
    ...
    """
    confusion_matrix = [[0 for j in range(0,IMG_WIDTH)] for i in range(0,IMG_HEIGHT)]
    ...
    for i in range(0,NUM_CLASSES):
        for j in range(0,NUM_CLASSES):
            ...
            if filled_count < 4:
                ...
                filled_count += 1
            elif float(ra[i][j]) > max(max_confusion_rate_val_ra):
...
def print_likelihoods_odds(tup, likelihood_ra):
...
def main(args):
    ...
    priors_ra, likelihood_ra = training(training_labels_file, training_images_file, k)
    training_end_time = time.time()
    testing_start_time = time.time()
    estimated_labels, highest_posterior_images_ra, lowest_posterior_images_ra = testing(priors_ra, likelihood_ra, test_images_file, test_labels_file)
    testing_end_time = time.time()
    actual_labels = read_file_into_line_ra(test_labels_file)

    classification_rates, overall_accuracy = compute_rate_correct(estimated_labels, actual_labels)
    ...
    print('Testing time: '),
    print(testing_end_time - testing_start_time)

    confusion_matrix = compute_confusion_matrix(estimated_labels, actual_labels)
