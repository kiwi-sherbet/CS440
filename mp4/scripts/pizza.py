...
def process_step(deliveryman, grid):
    ...
    if deliveryman.state == 2 or deliveryman.state == 3: # Have pizza
    ...
    for row in grid_rows:
        ...
        for c in row:
            if c == '%':
                ...
                markers.pizza.append((c_idx, r_idx))
            elif c == 'G':
                grid.ra[r_idx][c_idx] = 2
                markers.grocery.append((c_idx, r_idx))
            elif c == 'S':
                grid.ra[r_idx][c_idx] = 3
                markers.student.append((c_idx, r_idx))
            else:
...
def draw_board(screen, markers):
    ...
    draw_grocery_stores(screen, markers.grocery)
    draw_pizza_resaurants(screen, markers.pizza)
    draw_students(screen, markers.student)
...
def animation_main(size, policy_list, state, markers):
...
def main():
    ...
    utility_list, policy_list = iterate(grid, 10000)
#    value_list, policy_list = iterate(grid, 50)
    ...
    animation_main((grid.width, grid.height), policy_list, 0, markers)
    time.sleep(30)
    animation_main((grid.width, grid.height), policy_list, 1, markers)
    time.sleep(30)
    animation_main((grid.width, grid.height), policy_list, 2, markers)
    time.sleep(30)
    animation_main((grid.width, grid.height), policy_list, 3, markers)
    time.sleep(30)
