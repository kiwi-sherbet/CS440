...
def eat_dot1_ghost(maze_ra, start, arr_dots, ghost_start, ghost_initial_direction):
    ...
    while len(arr_dots) > 0:
        ...
        step, ghost_path, expanded, cost = a_star_ghost(maze_ra, current, target, ghost_start, ghost_initial_direction)
        total_expanded += expanded
        eat.append(target)
        total_ghost_path += ghost_path
        current = target
        for i in range(1, len(step)):
            soln_path.append(step[i])
    soln_path.insert(0, start)
...
def run_dijkstra_new(maze_ra, start, dots):
    visited = []
    frontier = []
    heapq.heappush(frontier, (0, start))
    for dot in dots:
        heapq.heappush(frontier, (sys.maxsize, dot))
    while len(visited) <= len(dots):
        curr = heapq.heappop(frontier)
        visited.append(curr)
        for dot in dots:
            if dot != curr:
                neighbor_node = get_tuple(frontier, visited, dot)
                new_cost = curr[0] + manhattan_dist(curr[1], dot)
                if new_cost < neighbor_node[0]:
                    new_cost_neighbor = (new_cost, dot)
...
def draw_dots(screen, dots):
    for x in dots:
        position = (SQUARE_SIZE * x[1], SQUARE_SIZE * x[0])
        #pygame.draw.rect(screen, (0, 60, 0), square)   
...
def write_number(screen, paths, left_dots, visited_dots):
    if len(paths) != 0:
        ...
        #text = font.render("H", True, (0, 128, 0))
...
def animation_main(size, walls, dots, soln_as_tuple_coords, ghost_as_tuple_coords):
    ...
    while running:
        ...
        write_number(screen, paths, left_dots, visited_dots)
...
def main():
    ...
    if alg == 'bfs':
        ...
        if len(ghost_start) == 0:
            ...
            soln_as_tuple_coords, ghost_as_tuple_coords, num_nodes_visited, path_cost = a_star_ghost(maze_ra, start, end, ghost_start, make_multi_ghost_direction(len(ghost_start)))
    elif alg == 'turns':
